...
function yearSelectPlugin(pluginConfig) {
    ...
    return function(fp) {
        ...
        const startYear = config.minDate ? new Date(config.minDate).getFullYear() : currentYear - 50;
        const endYear = config.maxDate ? new Date(config.maxDate).getFullYear() : currentYear + 50;
            ...
            if (selectedYears.length === 0) {
                ...
                if(config.maxRangeLength) {
                    if (maxYear - minYear > config.maxRangeLength){
                        if(startYear === minYear) maxYear = minYear + config.maxRangeLength;
                        if(startYear === maxYear) minYear = maxYear - config.maxRangeLength;
                    }
                }
        ...
        return {
            onReady: function() {
                // 기본 달력 숨기기
                fp.calendarContainer.querySelector(".flatpickr-months").style.display = "none";
                fp.calendarContainer.querySelector(".flatpickr-days").style.display = "none";
